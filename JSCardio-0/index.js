...
// 1. Filter the list of inventors for those who were born in the 1500's
    const fifteen = inventors.filter(inventor => inventor.year >= 1500 && inventor.year <1600); // true (keep it)
...
const peoples = [
  ...
  const isAdult = peoples.some(person => (new Date().getFullYear()) - person.year >= 19);
  console.log({isAdult})
  // Array.prototype.every() // is everyone 19 or older?
  const allAdults = peoples.every(person => (new Date().getFullYear()) - person.year >= 19);
...
//   companies.forEach(company => console.log(company))
  ...
  const retail = companies.filter(category => category.category == 'Retail');
  
  const eighties = companies.filter(company => company.start >= 1980);
